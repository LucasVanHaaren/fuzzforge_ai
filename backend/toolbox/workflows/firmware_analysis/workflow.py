"""
Firmware Analysis Workflow - Temporal Version
...
@workflow.defn
class FirmwareAnalysisWorkflow:
    """
    Analyze firmware images using unblob.

    User workflow:
    1. User runs: ff workflow run firmware_analysis .
    2. CLI uploads project to MinIO
    3. Worker downloads project
    4. Worker runs unblob analysis
    ...
    async def run(
        ...
        workflow.logger.info(
            f"Starting Firmware Analysis Workflow: "
        ...
        try:
            ...
            workflow.logger.info("Step 2: Running unblob")
            ...
            results["steps"].append({
                "step": "fuzzing",
                "status": "success",
                "files_extracted": fuzz_results.get("summary", {}).get("files_extracted", 0),
                "execution_time": fuzz_results.get("summary", {}).get("execution_time", 0)
            })
            workflow.logger.info(
                f"✓ Fuzzing completed: "
                f"{fuzz_results.get('summary', {}).get('files_extracted', 0)} files extracted"
            )


            # Step 3: Generate SARIF report from firmware_analysis results
            workflow.logger.info("Step 3: Generating SARIF report")
            try:
                sarif = await workflow.execute_activity(
                    "generate_sarif_report_firmware_analysis",
